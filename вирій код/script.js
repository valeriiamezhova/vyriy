...
// ---- Стартові блоки для нової сторінки ----
...
<section class="hero-block page-editable-block">
    ...
    <div class="hero-right">
        <div class="hero-info">
            <h1 class="block-text-placeholder">Ім'я Прізвище</h1>
            <p class="dates block-text-placeholder">дд.мм.рррр – дд.мм.рррр</p>
        </div>
    </div>
</section>
<section class="bio-block page-editable-block">
    ...
    <div class="bio-content">
        <div class="bio-column">
            <p class="block-text-placeholder">Розкажіть історію цієї людини: де народилась, чим займалась, що любила…</p>
        </div>
    </div>
</section>
<section class="human-world-section page-editable-block">
    <div class="human-world-container">
        <h2 class="section-title">СВІТ ЛЮДИНИ</h2>
        <div class="world-grid">
            <div class="world-card card-horizontal">
                <div class="sticker-frame">
                    <div class="sticker-frame-placeholder">
                        <button class="frame-add-photo-btn">Фото</button>
                        <button class="frame-add-sticker-btn">Стікер</button>
                ...
                <div class="card-body">
                    <h3>ДЕВІЗ:</h3>
                    <p class="block-text-placeholder">Улюблена фраза чи девіз…</p>
                </div>
            </div>
            <div class="world-card card-horizontal">
                <div class="sticker-frame">
                ...
                <div class="card-body">
                    <h3>ХОБІ:</h3>
                    <p class="block-text-placeholder">Захоплення та інтереси…</p>
                </div>
            </div>
        </div>
...
<div class="memory-section-block page-editable-block">
    <div class="memory-content">
        <h2>СПОГАДИ</h2>
        <p class="block-text-placeholder">Поділіться першим спогадом — словами, почуттями, моментом, який хочеться зберегти…</p>
    </div>
...
const _TOUR_STEPS = [
    {
        ...
        text: 'Тут усі доступні блоки — біографія, спогади, цитати, відео тощо. Натисніть на потрібний, щоб додати його на сторінку.',
        position: 'bottom'
    },
    {
        selector: null,
        text: 'Наведіть курсор на блок — з\'являться кнопки переміщення і видалення. Блоки також можна перетягувати мишкою, щоб змінити їх порядок на сторінці.',
        position: 'right'
    },
    {
        selector: '.hero-photo',
        text: 'Натисніть на будь-яке зображення на сторінці, щоб замінити його своїм.',
        position: 'right'
    },
    {
        selector: '[data-tour="decor"]',
        text: 'Стікери можна перетягувати на сторінку мишкою або просто натискати на них — вони одразу з\'являться на полотні. Потім їх можна вільно переміщати.',
        position: 'bottom'
    },
    {
        selector: '#theme-toggle',
        text: '☀️ Сонце — текст темний, для світлого фону. 🌙 Місяць — текст стає білим автоматично: обирайте цей режим, якщо фон сторінки темний, щоб текст залишався читабельним.',
        position: 'bottom'
    },
    {
        selector: '.world-add-card-btn',
        text: 'У блоці «Світ людини» можна додавати або прибирати картки (девіз, хобі, мрія, улюблена їжа тощо) — підлаштуйте їх кількість під те, скільки хочете розповісти.',
        position: 'top'
    },
    {
        selector: '[data-tour="layers"]',
        text: 'Тут можна переставляти елементи вище або нижче — це визначає, що буде зверху, а що під низом (наприклад, стікер поверх фотографії).',
        position: 'bottom'
    },
    {
        selector: '.btn-publish',
        text: 'Коли сторінка готова — опублікуйте і поділіться посиланням із рідними.',
        position: 'bottom'
    }
];
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (localStorage.getItem(_MEMORY_CREATED_KEY)) {
    ...
    if (tourStartBtn) tourStartBtn.addEventListener('click', _startTour);

    const tourSkipBtn = document.getElementById('tour-skip-btn');
    if (tourSkipBtn) tourSkipBtn.addEventListener('click', _skipTour);

    const tourCloseBtn = document.getElementById('tour-close-btn');
    if (tourCloseBtn) tourCloseBtn.addEventListener('click', _closeTour);

    const tourPrevBtn = document.getElementById('tour-prev-btn');
    if (tourPrevBtn) tourPrevBtn.addEventListener('click', _prevTourStep);

    const tourNextBtn = document.getElementById('tour-next-btn');
    if (tourNextBtn) tourNextBtn.addEventListener('click', _nextTourStep);

    const helpBtn = document.getElementById('tour-help-btn');
    if (helpBtn) helpBtn.addEventListener('click', _launchHelpTour);
});
...
// ---- Кнопка «Створити сторінку пам'яті» ----
...
function _startCreating() {
    ...
    setTimeout(function () {
        ...
        if (!localStorage.getItem(_TOUR_DONE_KEY)) {
            setTimeout(_showTourModal, 500);
        }
    }, 180);
}

function _setupStarterCanvas() {
    const container = document.querySelector('#page-canvas .container');
    if (!container) return;
    ...
    const canvas = document.getElementById('page-canvas');
    if (canvas) {
        canvas.querySelectorAll('.quote-cloud-section, .family-tribute-section').forEach(function (s) { s.remove(); });
    }
...
function _launchHelpTour() {
    _showTourModal();
}

function _showTourStep(index) {
    if (index >= _TOUR_STEPS.length) {
        _endTour();
        return;
    }

    const step = _TOUR_STEPS[index];
    ...
    if (counter) counter.textContent = 'Крок ' + (index + 1) + ' / ' + _TOUR_STEPS.length;
    if (prevBtn) prevBtn.style.visibility = index === 0 ? 'hidden' : 'visible';
    if (nextBtn) nextBtn.textContent = index === _TOUR_STEPS.length - 1 ? 'Завершити' : 'Далі →';
    ...
    if (tooltip) {
        ...
        requestAnimationFrame(function () {
            requestAnimationFrame(function () {
                if (target) {
                    _positionTourTooltip(tooltip, target, step.position);
                } else {
...
function _positionTourTooltip(tooltip, target, position) {
    ...
    var tw   = tooltip.offsetWidth  || 292;
